...
router.post('/produtos', async (req, res) => {
  try {
    ...
    const { nome, tipo, quantidade, estoque_minimo, custo_unitario, imagem_url } = req.body;
    ...
    if (quantidade > 0) {
      await queryTenant(
        ...
        [produto.id, profissional_id, quantidade]
      );
    }
...
router.post('/movimentacoes', async (req, res) => {
  try {
    ...
    const prodRes = await queryTenant(tenant_slug, 'SELECT quantidade FROM estoque_produtos WHERE id = $1', [produto_id]);
    if (prodRes.rows.length === 0) {
      return res.status(404).json({ error: 'Product not found.' });
    }

    const currentQty = parseInt(prodRes.rows[0].quantidade || 0, 10);
